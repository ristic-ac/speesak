def student_status_stats(df_complete_combined, poll =False):
    """
    Izračunava i prikazuje broj studenata grupisanih po koloni 'Način slušanja'.

    Argumenti:
        df_complete_combined (pandas.DataFrame): DataFrame koji sadrži podatke o studentima, uključujući kolonu 'Način slušanja'.
    ...
    Povratna vrednost:
        pandas.Series: Serija sa brojem studenata za svaku jedinstvenu vrednost u koloni 'Način slušanja'.
    """
...
def student_not_regular_in_group_stats(df_groups_combined, df_complete_combined):
    ...
    Argumenti:
        ...
        df_complete_combined (pd.DataFrame): DataFrame sa kompletnim podacima o studentima, koji sadrži kolone 'Broj indeksa' i 'Način slušanja'.
