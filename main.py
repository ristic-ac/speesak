...
df_poll = pd.read_excel("xlsx/DOMACI.xlsx")
df_poll = df_poll[["Ime", "Prezime", "Broj indeksa (npr. RA 123/2023)", "Obaveza"]]
...
# If there are any duplicates in "Broj indeksa" column, print them
df_poll = df_poll.drop_duplicates(subset="Broj indeksa", keep="last")

df_homework = df_poll[df_poll['Obaveza'] == "ДЗ"]
df_homework = df_homework.sort_values(by=["Broj indeksa"])
df_homework = df_homework.reset_index(drop=True)

df_sov = df_poll[df_poll['Obaveza'] == "СОВ"]
df_sov = df_sov.sort_values(by=["Broj indeksa"])
df_sov = df_sov.reset_index(drop=True)

print(df_homework)
print(df_sov)

# Export df_homework to schedules/homework.xlsx
df_homework.to_excel("schedules/homework.xlsx", index=False)
...
for xlsx_file, xlsx_k_file in xlsx_files:
    ...
    # Remove rows from df_groups and df_complete that have "Broj indeksa" that are in df_homework
    df_groups = df_groups[~df_groups['Broj indeksa'].isin(df_homework['Broj indeksa'])]
    df_groups = df_groups.reset_index(drop=True)
    df_groups = df_groups.dropna()
    df_complete = df_complete[~df_complete['Broj indeksa'].isin(df_homework['Broj indeksa'])]
    df_complete = df_complete.reset_index(drop=True)
    df_complete = df_complete.dropna()

    mutils.calculate_group_availability(STUDENTS_PER_GROUP, availability_by_groups, xlsx_file, df_groups)

    df_groups_combined = pd.concat([df_groups_combined, df_groups])
    df_complete_combined = pd.concat([df_complete_combined, df_complete])
...
print(df_groups_combined)


# # For each row in df_poll, find "Način polaganja" in df_complete_combined and add it to df_poll
# df_poll['Način polaganja'] = df_poll['Broj indeksa'].map(df_complete_combined.set_index('Broj indeksa')['Način polaganja'])
...
# Remove students from df_sov that are in df_groups_combined
df_sov = df_sov[~df_sov['Broj indeksa'].isin(df_groups_combined['Broj indeksa'])]
df_sov = df_sov.reset_index(drop=True)
df_sov['Način polaganja'] = df_sov['Broj indeksa'].map(df_complete_combined.set_index('Broj indeksa')['Način polaganja'])
print(df_sov)

dfs = [(status,students) for status, students in df_sov.groupby('Način polaganja')]
...
print("Residual students: ", residual_students)
print("Number of residual students: ", len(residual_students))
...
df_groups_combined = df_groups_combined[["RB", "Smer", "Grupa", "Broj indeksa", "Prezime", "Ime"]]
